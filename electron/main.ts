...
function loadBackupSettings() { try { if (!fs.existsSync(settingsPath())) return; const raw = JSON.parse(fs.readFileSync(settingsPath(), "utf8")); const value = Number(raw?.backupIntervalMinutes); if (Number.isFinite(value) && value >= 0) backupSettings.intervalMinutes = Math.min(1440, Math.floor(value)); } catch {} }
function saveBackupSettings() { fs.mkdirSync(app.getPath("userData"), { recursive: true }); fs.writeFileSync(settingsPath(), JSON.stringify({ backupIntervalMinutes: backupSettings.intervalMinutes }, null, 2), "utf8"); }
function backupDirectory() { return path.join(app.getPath("userData"), "backups"); }
function automaticBackup(reason: string) { try { saveDb(); if (!dbPath || !fs.existsSync(dbPath)) return { ok: false, error: "ظ‚ط§ط¹ط¯ط© ط§ظ„ط¨ظٹط§ظ†ط§طھ ط؛ظٹط± ط¬ط§ظ‡ط²ط©" }; const dir = backupDirectory(); fs.mkdirSync(dir, { recursive: true }); const stamp = new Date().toISOString().replace(/[:.]/g, "-"); const filePath = path.join(dir, `almaktaba-${reason}-${stamp}.sqlite`); fs.copyFileSync(dbPath, filePath); return { ok: true, path: filePath }; } catch (error) { console.error("automatic backup failed", error); return { ok: false, error: error instanceof Error ? error.message : String(error) }; } }
...
ipcMain.handle("auth:login", (_event,{username,password})=>{ const user=query("SELECT * FROM users WHERE username=? AND active=1",[username])[0]; if(!user||!verifyPassword(password,user.password_hash))return{ok:false,error:"ط§ط³ظ… ط§ظ„ظ…ط³طھط®ط¯ظ… ط£ظˆ ظƒظ„ظ…ط© ط§ظ„ظ…ط±ظˆط± ط؛ظٹط± طµط­ظٹط­ط©"}; session={id:Number(user.id),username:user.username,displayName:user.display_name,role:user.role}; audit("login","auth",null,"طھط³ط¬ظٹظ„ ط¯ط®ظˆظ„"); return{ok:true,user:session}; });
ipcMain.handle("auth:logout",()=>{session=null;return{ok:true};});
ipcMain.handle("auth:session",()=>session);
ipcMain.handle("auth:update-profile",(_event,{username,displayName})=>{if(!session)return{ok:false,error:"ط؛ظٹط± ظ…ط³ط¬ظ„ ط§ظ„ط¯ط®ظˆظ„"};const nextUsername=String(username||"").trim();const nextDisplayName=String(displayName||"").trim();if(!nextUsername||!nextDisplayName)return{ok:false,error:"ط§ط³ظ… ط§ظ„ظ…ط³طھط®ط¯ظ… ظˆط§ظ„ط§ط³ظ… ط§ظ„ط¸ط§ظ‡ط± ط­ظ‚ظˆظ„ ط¥ط¬ط¨ط§ط±ظٹط©"};if(!/^[A-Za-z0-9_]+$/.test(nextUsername))return{ok:false,error:"ط§ط³ظ… ط§ظ„ظ…ط³طھط®ط¯ظ… ظٹط¬ط¨ ط£ظ† ظٹط­طھظˆظٹ ط¹ظ„ظ‰ ط£ط­ط±ظپ ط¥ظ†ط¬ظ„ظٹط²ظٹط© ط£ظˆ ط£ط±ظ‚ط§ظ… ط£ظˆ _ ظپظ‚ط·"};const duplicate=query("SELECT id FROM users WHERE username=? AND id!=?",[nextUsername,session.id])[0];if(duplicate)return{ok:false,error:"ط§ط³ظ… ط§ظ„ظ…ط³طھط®ط¯ظ… ظ…ط³طھط®ط¯ظ… ط¨ط§ظ„ظپط¹ظ„"};const previous={username:session.username,displayName:session.displayName};db.run("UPDATE users SET username=?,display_name=? WHERE id=?",[nextUsername,nextDisplayName,session.id]);session={...session,username:nextUsername,displayName:nextDisplayName};audit("update","user",session.id,JSON.stringify({previous,newValues:{username:nextUsername,displayName:nextDisplayName}}));saveDb();return{ok:true,user:session};});
ipcMain.handle("auth:change-own-password",(_event,{oldPassword,newPassword})=>{if(!session)return{ok:false,error:"ط؛ظٹط± ظ…ط³ط¬ظ„ ط§ظ„ط¯ط®ظˆظ„"};if(String(newPassword||"").length<4)return{ok:false,error:"ظƒظ„ظ…ط© ط§ظ„ظ…ط±ظˆط± ط§ظ„ط¬ط¯ظٹط¯ط© ظٹط¬ط¨ ط£ظ† طھظƒظˆظ† 4 ط£ط­ط±ظپ/ط£ط±ظ‚ط§ظ… ط¹ظ„ظ‰ ط§ظ„ط£ظ‚ظ„"};const u=query("SELECT password_hash FROM users WHERE id=?",[session.id])[0];if(!u||!verifyPassword(oldPassword,u.password_hash))return{ok:false,error:"ظƒظ„ظ…ط© ط§ظ„ظ…ط±ظˆط± ط§ظ„ط­ط§ظ„ظٹط© ط؛ظٹط± طµط­ظٹط­ط©"};db.run("UPDATE users SET password_hash=? WHERE id=?",[hashPassword(newPassword),session.id]);audit("password_change","user",session.id,"طھط؛ظٹظٹط± ظƒظ„ظ…ط© ط§ظ„ظ…ط±ظˆط± ط§ظ„ط°ط§طھظٹط©");saveDb();return{ok:true};});
...
ipcMain.handle("transactions:create",(_event,input)=>{if(!session)return{ok:false,error:"ط؛ظٹط± ظ…ط³ط¬ظ„ ط§ظ„ط¯ط®ظˆظ„"};const type=String(input?.type||"").trim(),reason=String(input?.reason||"").trim(),amount=Number(input?.amount);if(!type||!reason||!Number.isFinite(amount)||amount<=0)return{ok:false,error:"ظ†ظˆط¹ ط§ظ„ط¹ظ…ظ„ظٹط© ظˆط§ظ„ظ…ط¨ظ„ط؛ ظˆط§ظ„طھط¨ط±ظٹط± ط­ظ‚ظˆظ„ ط¥ط¬ط¨ط§ط±ظٹط©"};const beneficiary=String(input?.beneficiary||"").trim(),notes=String(input?.notes||"").trim(),visibility=session.role==="admin"?"admin_private":"shop",now=new Date().toISOString();db.run("INSERT INTO transactions(type,amount,reason,beneficiary,notes,visibility,created_by,created_at) VALUES(?,?,?,?,?,?,?,?)",[type,amount,reason,beneficiary,notes,visibility,session.id,now]);const id=Number(query("SELECT last_insert_rowid() AS id")[0].id);audit("create","transaction",id,JSON.stringify({type,amount,reason,visibility}));saveDb();return{ok:true,id};});
ipcMain.handle("transactions:update",(_event,{id,input})=>{if(!session)return{ok:false,error:"ط؛ظٹط± ظ…ط³ط¬ظ„ ط§ظ„ط¯ط®ظˆظ„"};const t=query("SELECT * FROM transactions WHERE id=? AND status='active'",[id])[0];if(!t)return{ok:false,error:"ط§ظ„ط¹ظ…ظ„ظٹط© ط؛ظٹط± ظ…ظˆط¬ظˆط¯ط©"};if(session.role!=="admin"&&Number(t.created_by)!==session.id)return{ok:false,error:"ظ„ط§ ظٹظ…ظƒظ†ظƒ طھط¹ط¯ظٹظ„ ط¹ظ…ظ„ظٹط© ط³ط¬ظ„ظ‡ط§ ط§ظ„ظ…ط³طھط®ط¯ظ… ط§ظ„ط¢ط®ط±"};if(t.visibility==='admin_private'&&session.role!=="admin")return{ok:false,error:"ظ‡ط°ظ‡ ط§ظ„ط¹ظ…ظ„ظٹط© ط®ط§طµط©"};const type=String(input?.type||"").trim(),reason=String(input?.reason||"").trim(),amount=Number(input?.amount);if(!type||!reason||!Number.isFinite(amount)||amount<=0)return{ok:false,error:"ظ†ظˆط¹ ط§ظ„ط¹ظ…ظ„ظٹط© ظˆط§ظ„ظ…ط¨ظ„ط؛ ظˆط§ظ„طھط¨ط±ظٹط± ط­ظ‚ظˆظ„ ط¥ط¬ط¨ط§ط±ظٹط©"};const beneficiary=String(input?.beneficiary||"").trim(),notes=String(input?.notes||"").trim(),visibility=String(t.visibility)==='admin_private'?'admin_private':'shop';db.run("UPDATE transactions SET type=?,amount=?,reason=?,beneficiary=?,notes=?,visibility=?,updated_at=? WHERE id=?",[type,amount,reason,beneficiary,notes,new Date().toISOString(),id]);audit("update","transaction",id,JSON.stringify({previous:t,newValues:{type,amount,reason}}));saveDb();return{ok:true};});
ipcMain.handle("transactions:void",(_event,{id,reason})=>{if(!session)return{ok:false,error:"ط؛ظٹط± ظ…ط³ط¬ظ„ ط§ظ„ط¯ط®ظˆظ„"};const t=query("SELECT * FROM transactions WHERE id=? AND status='active'",[id])[0];if(!t)return{ok:false,error:"ط§ظ„ط¹ظ…ظ„ظٹط© ط؛ظٹط± ظ…ظˆط¬ظˆط¯ط©"};if(session.role!=="admin"&&Number(t.created_by)!==session.id)return{ok:false,error:"ظ„ط§ ظٹظ…ظƒظ†ظƒ ط¥ظ„ط؛ط§ط، ط¹ظ…ظ„ظٹط© ط§ظ„ظ…ط³طھط®ط¯ظ… ط§ظ„ط¢ط®ط±"};db.run("UPDATE transactions SET status='void',void_reason=?,voided_by=?,voided_at=? WHERE id=?",[String(reason||"ط¨ط¯ظˆظ† ط³ط¨ط¨").trim(),session.id,new Date().toISOString(),id]);audit("void","transaction",id,String(reason||"ط¨ط¯ظˆظ† ط³ط¨ط¨").trim());saveDb();return{ok:true};});
...
ipcMain.handle("products:create",(_event,input)=>{if(!session)return{ok:false,error:"ط؛ظٹط± ظ…ط³ط¬ظ„ ط§ظ„ط¯ط®ظˆظ„"};const name=String(input?.name||"").trim();let sku=String(input?.sku||"").trim();const unit=String(input?.unit||"ظ‚ط·ط¹ط©").trim()||"ظ‚ط·ط¹ط©",purchase=Number(input?.purchasePrice),sale=Number(input?.salePrice),quantity=Number(input?.quantity),min=Number(input?.minQuantity);if(!name)return{ok:false,error:"ط§ط³ظ… ط§ظ„ظ…ظ†طھط¬ ط¥ط¬ط¨ط§ط±ظٹ"};if([purchase,sale,quantity,min].some(v=>!Number.isFinite(v)||v<0))return{ok:false,error:"طھط­ظ‚ظ‚ ظ…ظ† ط§ظ„ط£ط³ط¹ط§ط± ظˆط§ظ„ظƒظ…ظٹط§طھ"};if(sku){const duplicate=query("SELECT id FROM products WHERE sku=? AND active=1",[sku])[0];if(duplicate)return{ok:false,error:"ط±ظ…ط² ط§ظ„ظ…ظ†طھط¬ ظ…ط³طھط®ط¯ظ… ط¨ط§ظ„ظپط¹ظ„"};}else sku=nextProductCode();const now=new Date().toISOString();db.run("INSERT INTO products(name,sku,unit,purchase_price,sale_price,quantity,min_quantity,created_at) VALUES(?,?,?,?,?,?,?,?)",[name,sku,unit,purchase,sale,quantity,min,now]);const id=Number(query("SELECT last_insert_rowid() AS id")[0].id);audit("create","product",id,JSON.stringify({name,sku,unit,purchase,sale,quantity,min}));saveDb();return{ok:true,id,sku};});
ipcMain.handle("products:update",(_event,{id,input})=>{if(!session)return{ok:false,error:"ط؛ظٹط± ظ…ط³ط¬ظ„ ط§ظ„ط¯ط®ظˆظ„"};const p=query("SELECT * FROM products WHERE id=? AND active=1",[id])[0];if(!p)return{ok:false,error:"ط§ظ„ظ…ظ†طھط¬ ط؛ظٹط± ظ…ظˆط¬ظˆط¯"};const name=String(input?.name||"").trim(),sku=String(input?.sku||"").trim(),unit=String(input?.unit||"ظ‚ط·ط¹ط©").trim()||"ظ‚ط·ط¹ط©",purchase=Number(input?.purchasePrice),sale=Number(input?.salePrice),quantity=Number(input?.quantity),min=Number(input?.minQuantity);if(!name)return{ok:false,error:"ط§ط³ظ… ط§ظ„ظ…ظ†طھط¬ ط¥ط¬ط¨ط§ط±ظٹ"};if([purchase,sale,quantity,min].some(v=>!Number.isFinite(v)||v<0))return{ok:false,error:"طھط­ظ‚ظ‚ ظ…ظ† ط§ظ„ط£ط³ط¹ط§ط± ظˆط§ظ„ظƒظ…ظٹط§طھ"};const duplicate=sku?query("SELECT id FROM products WHERE sku=? AND id!=? AND active=1",[sku,id])[0]:null;if(duplicate)return{ok:false,error:"ط±ظ…ط² ط§ظ„ظ…ظ†طھط¬ ظ…ط³طھط®ط¯ظ… ط¨ط§ظ„ظپط¹ظ„"};const previous={name:p.name,sku:p.sku,unit:p.unit,purchasePrice:p.purchase_price,salePrice:p.sale_price,quantity:p.quantity,minQuantity:p.min_quantity};db.run("UPDATE products SET name=?,sku=?,unit=?,purchase_price=?,sale_price=?,quantity=?,min_quantity=?,updated_at=? WHERE id=?",[name,sku,unit,purchase,sale,quantity,min,new Date().toISOString(),id]);audit("update","product",id,JSON.stringify({previous,newValues:{name,sku,unit,purchasePrice:purchase,salePrice:sale,quantity,minQuantity:min}}));saveDb();return{ok:true};});
ipcMain.handle("products:delete",(_event,id)=>{if(!session)return{ok:false,error:"ط؛ظٹط± ظ…ط³ط¬ظ„ ط§ظ„ط¯ط®ظˆظ„"};const p=query("SELECT * FROM products WHERE id=? AND active=1",[id])[0];if(!p)return{ok:false,error:"ط§ظ„ظ…ظ†طھط¬ ط؛ظٹط± ظ…ظˆط¬ظˆط¯"};db.run("UPDATE products SET active=0,updated_at=? WHERE id=?",[new Date().toISOString(),id]);audit("delete","product",Number(id),JSON.stringify({name:p.name,sku:p.sku}));saveDb();return{ok:true};});
...
ipcMain.handle("system:backup",()=>{if(!session||session.role!=="admin")return{ok:false,error:"ظ‡ط°ظ‡ ط§ظ„ط¹ظ…ظ„ظٹط© ظ…طھط§ط­ط© ظ„ظ„ط­ط³ط§ط¨ ط§ظ„ط¥ط¯ط§ط±ظٹ ظپظ‚ط·"};saveBackupSettings();return{ok:true,intervalMinutes:backupSettings.intervalMinutes};});
ipcMain.handle("system:backup-now",()=>{if(!session||session.role!=="admin")return{ok:false,error:"ظ‡ط°ظ‡ ط§ظ„ط¹ظ…ظ„ظٹط© ظ…طھط§ط­ط© ظ„ظ„ط­ط³ط§ط¨ ط§ظ„ط¥ط¯ط§ط±ظٹ ظپظ‚ط·"};return automaticBackup("manual");});
ipcMain.handle("system:backup-settings",()=>({ok:true,intervalMinutes:backupSettings.intervalMinutes}));
ipcMain.handle("system:set-backup-settings",(_event,{intervalMinutes})=>{if(!session||session.role!=="admin")return{ok:false,error:"ظ‡ط°ظ‡ ط§ظ„ط¹ظ…ظ„ظٹط© ظ…طھط§ط­ط© ظ„ظ„ط­ط³ط§ط¨ ط§ظ„ط¥ط¯ط§ط±ظٹ ظپظ‚ط·"};const value=Math.max(0,Math.min(1440,Math.floor(Number(intervalMinutes)||0)));backupSettings.intervalMinutes=value;saveBackupSettings();restartBackupTimer();return{ok:true,intervalMinutes:value};});
ipcMain.handle("backup:list",()=>{if(!session||session.role!=="admin")return{ok:false,error:"ظ‡ط°ظ‡ ط§ظ„ط¹ظ…ظ„ظٹط© ظ…طھط§ط­ط© ظ„ظ„ط­ط³ط§ط¨ ط§ظ„ط¥ط¯ط§ط±ظٹ ظپظ‚ط·"};const dir=backupDirectory();if(!fs.existsSync(dir))return{ok:true,backups:[]};const backups=fs.readdirSync(dir,{withFileTypes:true}).filter(e=>e.isFile()&&e.name.toLowerCase().endsWith(".sqlite")).map(e=>{const filePath=path.join(dir,e.name);const stat=fs.statSync(filePath);return{name:e.name,size:stat.size,modifiedAt:stat.mtime.toISOString()};}).sort((a,b)=>b.modifiedAt.localeCompare(a.modifiedAt));return{ok:true,backups};});
ipcMain.handle("backup:restore",async(_event,name?:string)=>{if(!session||session.role!=="admin")return{ok:false,error:"ط§ط³طھط¹ط§ط¯ط© ظ‚ط§ط¹ط¯ط© ط§ظ„ط¨ظٹط§ظ†ط§طھ ظ…طھط§ط­ط© ظ„ظ„ط­ط³ط§ط¨ ط§ظ„ط¥ط¯ط§ط±ظٹ ظپظ‚ط·"};const dir=backupDirectory();let filePath="";if(name){const safeName=path.basename(String(name));filePath=path.join(dir,safeName);}else{const owner=mainWindow ?? undefined;if(!owner)return{ok:false,error:"ظ†ط§ظپط°ط© ط§ظ„ط¨ط±ظ†ط§ظ…ط¬ ط؛ظٹط± ط¬ط§ظ‡ط²ط©"};const picked=await dialog.showOpenDialog(owner,{title:"ط§ط®طھظٹط§ط± ظ†ط³ط®ط© ط§ط­طھظٹط§ط·ظٹط©",properties:["openFile"],filters:[{name:"ظ‚ط§ط¹ط¯ط© ط¨ظٹط§ظ†ط§طھ SQLite",extensions:["sqlite"]}]});if(picked.canceled||!picked.filePaths[0])return{ok:false,canceled:true};filePath=picked.filePaths[0];}if(!fs.existsSync(filePath))return{ok:false,error:"ظ…ظ„ظپ ط§ظ„ظ†ط³ط®ط© ط§ظ„ط§ط­طھظٹط§ط·ظٹط© ط؛ظٹط± ظ…ظˆط¬ظˆط¯"};try{const current=automaticBackup("before-restore");if(!current.ok)return current;fs.copyFileSync(filePath,dbPath);saveDb();app.relaunch();app.exit(0);return{ok:true};}catch(error){return{ok:false,error:error instanceof Error?error.message:String(error)};}});
  ...
  autoUpdater.on("update-available", async (info) => {
    try {
      const result = await dialog.showMessageBox(mainWindow!, {
        type: "info",
        title: "\u062a\u062d\u062f\u064a\u062b \u062c\u062f\u064a\u062f",
        message: "\u064a\u0648\u062c\u062f \u0625\u0635\u062f\u0627\u0631 \u062c\u062f\u064a\u062f \u0645\u0646 \u0628\u0631\u0646\u0627\u0645\u062c \u0627\u0644\u0645\u0643\u062a\u0628\u0629.",
  ...
  autoUpdater.on("update-downloaded", async () => {
    try {
      const result = await dialog.showMessageBox(mainWindow!, {
        ...
        detail: "\u0633\u064a\u062a\u0645 \u0625\u0639\u0627\u062f\u0629 \u062a\u0634\u063a\u064a\u0644 \u0627\u0644\u0628\u0631\u0646\u0627\u0645\u062c \u0644\u062a\u062b\u0628\u064a\u062a \u0627\u0644\u0625\u0635\u062f\u0627\u0631 \u0627\u0644\u062c\u062f\u064a\u062f.",
      ...
      console.error("update install prompt failed", error);
    }
  });
